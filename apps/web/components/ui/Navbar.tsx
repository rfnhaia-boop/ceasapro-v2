...
export default function Navbar({ user }: { user: any }) {
  ...
  const navItems = [
    ...(role === 'ADMIN' ? [{ path: '/admin', icon: LayoutDashboard, label: 'Painel' }] : []),
    ...(role !== 'DRIVER' ? [{ path: '/picker', icon: Package, label: 'Coletas' }] : []),
    ...(role !== 'PICKER' ? [{ path: '/deliveries', icon: Truck, label: 'Entregas' }] : []),
  ]
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:block text-right">
              ...
              <p className="text-[10px] uppercase tracking-widest text-slate-500">
                {role === 'ADMIN' ? 'Admin' : role === 'PICKER' ? 'Separador' : 'Entregador'}
              </p>
            </div>
            ...
            <button
              ...
              className="p-2 text-slate-400 hover:text-orange-400 transition-colors rounded-xl hover:bg-slate-800"
            >
      ...
      {/* Bottom nav mobile */}
      <nav className="md:hidden fixed bottom-0 inset-x-0 bg-slate-950/95 backdrop-blur-md border-t border-slate-800 flex justify-around p-2 z-50">
        {navItems.map(({ path, icon: Icon, label }) => (
          <Link key={path} href={path} className={cn(
            ...
            <Icon className={cn('w-6 h-6 mb-1', isActive(path) ? 'text-emerald-400' : 'text-slate-500')} />
            {label}
          </Link>
        ))}
      </nav>
